...
import 'servicediscovery/ItemsDiscoveryNegotiator.dart';
...
class ConnectionNegotiatorManager {
  ...
  void negotiateFeatureList(xml.XmlElement element) {
    ...
    if (_connection.authenticated) {
      ...
      // http_file_upload https://xmpp.org/extensions/xep-0363.html
      waitingNegotiators.add(NegotiatorWithSupportedNonzas(
          ItemsDiscoveryNegotiator.getInstance(_connection), []));
    }
  ...
  void negotiateNextFeature() {
    ...
    if (negotiatorWithData != null) {
      ...
      try {
        ...
        Log.e(TAG, 'Stream has already been listened to this listener ${e.toString()}');
      }
    } else {
      activeNegotiator = null;
  ...
  void _initSupportedNegotiatorList() {
    ...
    // http_file_upload https://xmpp.org/extensions/xep-0363.html
    // supportedNegotiatorList.add(ItemsDiscoveryNegotiator.getInstance(_connection));
  }
  ...
  NegotiatorWithSupportedNonzas pickNextNegotiator() {
    ...
    var negotiatorWithData = waitingNegotiators.firstWhere((element) {
      Log.d(TAG,
          '${element.negotiator.expectedName} Found matching negotiator ${element.negotiator.isReady().toString()}');
