...
class MAMNegotiator extends Negotiator {
  ...
  static const XMLNS_MAM = 'urn:xmpp:mam:2';
  ...
  List<Nonza> match(List<Nonza> requests) {
    return requests
        .where((element) =>
            element is Feature &&
            ((element).xmppVar == XMLNS_MAM ||
                (element).xmppVar == '$XMLNS_MAM#extended'))
        .toList();
  }
  ...
  void sendRequest() {
    ...
    query.addAttribute(XmppAttribute('xmlns', XMLNS_MAM));
  ...
  void checkForExtendedSupport(List<Nonza> nonzas) {
    hasExtended = nonzas.any(
        (element) => (element as Feature).xmppVar == '$XMLNS_MAM#extended');
  }
}
