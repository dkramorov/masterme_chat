...
class StreamManagementModule extends Negotiator {
  ...
  bool ackTurnedOn = true; // Включен псевдо-пинг connection.account.ackEnabled
  int lastResponse; // DateTime.now().millisecondsSinceEpoch 1sec = 1000
  Timer timer;
  ...
  void sendAckRequest() {
    if (ackTurnedOn) {
      /* Каждые 5 секунд пуляем запрос на сервер
      * <r xmlns="urn:xmpp:sm:3"/>
      * получаем <a h='7' xmlns='urn:xmpp:sm:3'/>
      * Если долго нихера не получаем, тогда надо выставить флаг
      */
      if (lastResponse != null) {
        LATENCY = DateTime.now().millisecondsSinceEpoch - lastResponse;
        if (LATENCY > _connection.account.maxReconnectLatency) {
          Log.w(TAG, 'latency $LATENCY');
          //print('CONNECTION reached ${_connection.account.maxReconnectLatency} LATENCY');
          //print('DISCONNECTING...');
          //_connection.close();
        }
      }
  ...
  void parseAckResponse(String rawValue) {
    lastResponse = DateTime.now().millisecondsSinceEpoch;
  ...
  void resumeState(Nonza resumedNonza) {
    ...
    // Вызовет очистку всех неготиаторов,
    // все они в IDLE перейдут - это хрень какая то
