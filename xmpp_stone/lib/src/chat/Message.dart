...
class Message {
    ...
    try {
      ...
      if (forwarded != null) {
        ...
        if (message != null) {
          ...
          var url = message.getChild('url')?.textValue;
          var urlType = message.getChild('urlType')?.textValue;
          var localId = message.getChild('localId')?.textValue;
  ...
  static Message _parseArchived(MessageStanza stanza) {
    var result = stanza.children.firstWhere(
        ...
        orElse: () => null);
    try {
      var mam = result?.getNameSpace() == MAMNegotiator.XMLNS_MAM;
  ...
  static Message _parseRegularMessage(MessageStanza stanza) {
    var messageId = stanza.getChild('stanza-id')?.getAttribute('id')?.value;
    return Message(
      stanza,
      stanza.toJid,
      stanza.fromJid,
      ...
      url: stanza.url,
      urlType: stanza.urlType,
      localId: stanza.localId,
      isMam: false,
      messageId: messageId,
    );
  }
  ...
  set isMam(bool value) {
    _isMam = value;
  }

  String get url => _url;

  String get urlType => _urlType;

  String get localId => _localId;

  bool get isMam => _isMam;
