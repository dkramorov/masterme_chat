...
class Connection {
  ...
  Connection(this.account) {
    ...
    HttpFileUploadManager.getInstance(this);
...
""";
  ...
  // --------------------------------
  // TCP KEEPALIVE RAW SOCKET OPTIONS
  // --------------------------------
  void setKeepAliveSocketRawOptions() {
    Log.w(TAG, 'SET KEEPALIVE SOCKET RAW OPTIONS for ${Platform.operatingSystem}');
    if (Platform.isIOS || Platform.isMacOS) {
      var rawSocketOptionKeepALiveIOS = RawSocketOption.fromBool(
        /* SOL_SOCKET */
        0xffff,
        /* SO_KEEPALIVE */
        0x0008,
      ...
      var rawSocketOptionPeriodIOS = RawSocketOption.fromInt(
        /* SOL_SOCKET */
        6,
        /* TCP_KEEPALIVE */
        0x10,
      ...
      _socket.setRawOption(rawSocketOptionKeepALiveIOS);
      _socket.setRawOption(rawSocketOptionPeriodIOS);
    } else if (Platform.isAndroid) {
      var rawSocketOptionKeepAliveAndroid = RawSocketOption.fromBool(
        /* SOL_SOCKET */
        0x1,
        /* SO_KEEPALIVE */
        0x0009,
      ...
      var rawSocketOptionPeriodAndroid = RawSocketOption.fromInt(
        /* IPPROTO_TCP */
        6,
        /* TCP_KEEPIDLE */
        4,
      ...
      _socket.setRawOption(rawSocketOptionKeepAliveAndroid);
      _socket.setRawOption(rawSocketOptionPeriodAndroid);
    }
  }
        ...
        if (_state != XmppConnectionState.Closed) {
          ...
          socket
              ...
              .listen(handleResponse,
                  onDone: handleConnectionDone, onError: handleSocketOnError);
          _openStream();
        } else {
...
//      });
  ...
  void processError(xml.XmlDocument xmlResponse) {
    ...
    Log.e(TAG, '[ERROR]: ${xmlResponse.toString()}');
  }
  ...
  void handleSocketOnError(Object error) {
    Log.e(TAG, 'Handle socket on Error: ${error.toString()}');
    handleCloseState();
  }

  void handleConnectionError(String error) {
    Log.e(TAG, 'Handle error connection: $error');
    handleCloseState();
  }
